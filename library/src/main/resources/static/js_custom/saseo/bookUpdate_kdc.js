...
function kdcCallSignLoad(){
	...
	const totalAmount = $('#book_totalAmount').val();
	...
	*/
	...
	for(var i = 0; i < totalAmount_Integer; i++){
		...
		kdcCallSignListHtml += `<tr id = "trId`+trIdNumber+`">
		 										<td colspan="2"> `+samebookArray[i].kdcCallSign+`</td>  `;
				
		
		// 여기부터 ★2열 정의함★
	    kdcCallSignListHtml +=`      <td colspan="2" >
												    	<!-- 새로 입력하는 곳 -->
												    	<div class="booking-form-i  inputBox-custom"  >
															<div class="input">
																<input type="text"  name="kdcCallSign" value= `+samebookArray[i].kdcCallSign+` class="kdcCallSignClass" placeholder="청구기호를 입력하세요">
															</div>
														</div>
														<div style="text-align: -webkit-center;">
...
function kdcAdd(event){
	...
	const kdcTrAddHtml = `<tr id="trId`+kdcTrCount+`">
												...
												<td colspan="2" >
											    	...
											    	<div style="text-align: -webkit-center;">
														<button onclick ="kdcRemove(trId`+kdcTrCount+` )"  class=" booking-complete-btn white-btn-custom" >제거</button>
													</div>
											    </td>
											</tr>`
